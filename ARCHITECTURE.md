...
- **Render guard** (`ui_config.render`): everything between `imgui.Begin` and `imgui.End` runs inside a `pcall`, the same tolerance `automation.lua` gives a throwing action module. A Lua error escaping the body would leave `Begin` open — and in tab mode `BeginTabBar`, usually with a `BeginTabItem` inside it — and ImGui asserts on an unbalanced stack rather than tolerating one, so a recoverable Lua fault became a hard client failure. On a caught error `components.abort_sections()` closes whatever tab chrome is still open (it reads the file-local `tab_bar_open`/`tab_item_open` that `begin_sections`/`begin_tab_section` set, takes no `ctx` since the frame's is discarded either way, and is idempotent — it clears each flag as it pops, so a second call is a no-op rather than a double `EndTabItem`), the `End()`/`PopStyleVar()` **outside** the guard balance the window, and the frame renders short. The scope is exactly the chrome `config.lua` opens itself: a `PushStyleColor`/`PushID`/`BeginGroup` left unpopped deeper inside `components.lua` is still unbalanced at `End`, because nothing in Ashita's ImGui binding reports a stack depth to unwind against — closing that would mean `components.lua` counting its own pushes. The `render_widget` and `panel.lua` windows are **not** guarded. Because the same fault repeats at frame rate, `last_render_error` prints only the first of a run of identical messages; it is cleared on the first clean frame and in `ui_config.show()`, so a fault that survives a close/reopen is reported again instead of being silently swallowed.
...
- **UI isolation**: `pcall` wraps the config window body in `config.lua`, with `components.abort_sections()` unwinding the tab chrome on a caught error. See "config.lua – Orchestration" for what it does and does not cover.
